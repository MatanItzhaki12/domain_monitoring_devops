...
def test_1_login_invalid_wrong_details(driver, base_url, username, password):
...
def test_2_login_invalid_empty_fields(driver, base_url, username, password):
    login_page = LoginPage(driver, base_url)
    ...
    # Check for error message visibility and content
    assert login_page.get_text(login_page.error_locator) == "username and password are required"
    assert login_page.is_visible(login_page.error_locator) == True
...
# --------
def test_3_login_valid(driver, base_url):
    login_page = LoginPage(driver, base_url)
